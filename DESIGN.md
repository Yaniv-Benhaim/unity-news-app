...
The same AIDL contract files and DTO classes are kept in both projects. Contract unit tests cover request serialization and DTO behavior so drift is caught during normal test runs.
...
- Carries generic filter values through IPC and cache keys.
...
- Verifies AIDL request serialization behavior.
